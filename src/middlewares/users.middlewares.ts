    ...
    options: async (value: string, { req }) => {
      ...
      try {
        ...
        if (user.forgot_password_token !== value) {
          throw new ErrorWithStatus({
            message: USERS_MESSAGES.FORGOT_PASSWORD_TOKEN_IN_VALID,
            status: HTTP_STATUS.UNAUTHORIZED
          })
        }
  ...
  checkSchema(
    {
      email: {
        isEmail: {
          errorMessage: USERS_MESSAGES.EMAIL_IN_VALID
        },
        trim: true,
        custom: {
          options: async (value, { req }) => {
          ...
          options: async (value: string, { req }) => {
            ...
            try {
              ...
              if (refresh_token === null) {
                throw new ErrorWithStatus({
                  message: USERS_MESSAGES.USED_REFRESH_TOKEN_NO_EXIST,
                  status: HTTP_STATUS.UNAUTHORIZED
                })
              }
      ...
      email_verify_token: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            if (!value) {
              throw new ErrorWithStatus({
                message: USERS_MESSAGES.EMAIL_VERIFIED_TOKEN_IS_REQUIRED,
                status: HTTP_STATUS.UNAUTHORIZED
              })
            }
            try {
      ...
      username: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            ...
            //if user exists in database, update is not allowed
            if (user) {
